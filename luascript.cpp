...
int32_t LuaInterface::luaDoCreatureSay(lua_State* L)
{
	...
	if(params > 5)
		lua_pushboolean(L, g_game.internalCreatureSay(creature, type, text, ghost, &list, &pos, 0, false));
	else
		lua_pushboolean(L, g_game.internalCreatureSay(creature, type, text, ghost, &list, NULL, 0, false));
