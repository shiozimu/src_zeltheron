...
class ProtocolGame : public Protocol
{
	public:
...
#endif
			player = NULL;
			eventConnect = m_packetCount = m_packetTime, m_lastSwitch = 0;
		...
		void spectate(const std::string& name, const std::string& password, uint16_t version);
	...
	private:
		...
		bool lookAt(uint32_t playerId, const Position& pos, uint16_t spriteId, int16_t stackpos);
		...
		void GetFloorDescription(OutputMessage_ptr msg, int32_t x, int32_t y, int32_t z,
			int32_t width, int32_t height, int32_t offset, int32_t& skip, bool storageDynamic = false);

		//translate a map area to clientreadable format
		void GetMapDescription(int32_t x, int32_t y, int32_t z,
			int32_t width, int32_t height, OutputMessage_ptr msg, bool storageDynamic = false);

		void AddMapDescription(OutputMessage_ptr msg, const Position& pos, bool storageDynamic = false);
		...
		//shop
		void AddShopItem(OutputMessage_ptr msg, const ShopInfo& item, uint32_t version);
