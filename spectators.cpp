...
void Spectators::addSpectator(ProtocolGame* client)
{
	...
	Database* db = Database::getInstance();
	DBResult_ptr description;
	DBQuery query;
	std::string descriptionStr;
	
	query << "SELECT `castDescription` FROM `players` WHERE `id` = " << owner->getPlayer()->getGUID() << ";";
	if(!(description = db->storeQuery(query.str())))
		descriptionStr = "Welcome to the Cast System!\nUse Ctrl + Arrows to navigate between other streams.";
	else
		if (description->getDataString("castDescription") == "")
			descriptionStr = "Welcome to the Cast System!\nUse Ctrl + Arrows to navigate between other streams.";
		else	
			descriptionStr = description->getDataString("castDescription");

	client->sendCreatureSay(owner->getPlayer(), MSG_PRIVATE, descriptionStr, NULL, 0);
}
