...
ReturnValue Actions::canUse(const Player* player, const Position& pos)
{
    ...
    if(tile)
    {
        ...
        if(houseTile && houseTile->getHouse())
        {
            ...
            if(!houseTile->getHouse()->isInvited(player))
...
bool Actions::useItem(Player* player, const Position& pos, uint8_t index, Item* item)
{
	...
	if(tile)
    {
        ...
        if(houseTile && houseTile->getHouse())
        {        
            Player* houseOwner = g_game.getPlayerByGuidEx(houseTile->getHouse()->getOwner()); 

            //Guild Halls:
            uint32_t owner = houseTile->getHouse()->getOwner();
            if (houseTile->getHouse()->isGuild() && IOGuild::getInstance()->swapGuildIdToOwner(owner)){
        		houseOwner = g_game.getPlayerByGuidEx(owner);
            }   

            //Verificar Storage
            if(houseOwner && houseOwner != player && !player->hasCustomFlag(PlayerCustomFlag_CanThrowAnywhere))
            {
                std::string blockItems;
                houseOwner->getStorage("65478987", blockItems);
                if (atoi(blockItems.c_str()) > 0)
                	// Items can use
                	if (item->getID() != 1386 && item->getID() != 3678) {
                    	player->sendCancelMessage(RET_HOUSEPROTECTED);    
                    	return false;
                    }
            }       
        }
	...
	uint8_t toStackPos, Item* item, bool isHotkey, uint32_t creatureId/* = 0*/)
{
	...
	Tile* tile = g_game.getTile(fromPos);
	if(tile)
    {
        HouseTile* houseTile = tile->getHouseTile();
        if(houseTile && houseTile->getHouse())
	...
	if (item->getID() == 2285) {
		...
		player->setNextActionTask(NULL);
		//player->stopWalk();
		player->setNextExAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::CUSTOM_ACTIONS_DELAY_INTERVAL) + 2000);
	} else {
		if(!player->canDoAction())
			return false;
	...
	if (item->getID() == 2289) {
		...
		player->setNextActionTask(NULL);
		//player->stopWalk();
		player->setNextExAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::CUSTOM_ACTIONS_DELAY_INTERVAL) + 1000);
	} else {
		if(!player->canDoAction())
			return false;
	...
	if (item->getID() == 2293) {
		...
		player->setNextActionTask(NULL);
		//player->stopWalk();
		player->setNextExAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::CUSTOM_ACTIONS_DELAY_INTERVAL) + 700);
	} else {
		if(!player->canDoAction())
			return false;
